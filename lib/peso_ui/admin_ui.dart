...
List<JobSeekerUser> _generateJobSeekerUsers(List<Applicant> applicants) {
  ...
  for (final a in applicants) {
    ...
    final lastLoginDaysAgo = isActive
        ? (rand.next() * 3).floor()
        : 4 + (rand.next() * 20).floor();
...
List<Concern> _generateConcerns(List<Applicant> applicants) {
  ...
  final notMatched = applicants
      .where((a) => a.matchStatus == 'not_matched')
      .toList();
...
class _NavItem {
  ...
  const _NavItem(this.tab, this.label, this.icon, this.title, this.description);
}
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(24),
          border: Border.all(
            color: _hover ? widget.color.withValues(alpha: 0.35) : _C.slate200,
          ),
          boxShadow: [
            BoxShadow(
...
class _MultiLinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final stepX = dates.length > 1
        ? size.width / (dates.length - 1)
        : size.width;
      ...
      for (var i = 0; i < s.values.length; i++) {
        ...
        if (isHover) {
          canvas.drawCircle(
            Offset(x, y),
            8,
            Paint()..color = s.color.withValues(alpha: 0.20),
          );
        }
      }
    }
...
class _TrendMiniCardState extends State<_TrendMiniCard> {
  ...
  Widget _buildTooltip(double width) {
    ...
    final stepX = widget.dates.length > 1
        ? width / (widget.dates.length - 1)
        : width;
    final x = idx * stepX;
    ...
    return Positioned(
      ...
      right: alignRight
          ? (width - x - 10).clamp(0.0, math.max(0.0, width - 20))
          : null,
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              _fmtShortDate(widget.dates[idx]),
              style: const TextStyle(
                fontSize: 10.5,
                color: _C.slate500,
                fontWeight: FontWeight.w700,
            ...
            Text(
              '${widget.label} : ${widget.values[idx].toInt()}',
              style: TextStyle(
                fontSize: 11.5,
                fontWeight: FontWeight.w800,
                color: widget.color,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (hoverIndex != null && hoverIndex! >= 0 && hoverIndex! < points.length) {
      ...
      while (startY < size.height) {
        canvas.drawLine(
          Offset(hp.dx, startY),
          Offset(hp.dx, math.min(startY + dashH, size.height)),
          dashPaint,
        );
        startY += dashH + dashG;
      }
...
class _ApplicantAnalyticsSectionState extends State<ApplicantAnalyticsSection> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final d in dates) {
      final dayApplicants = filtered.where(
        (a) => _isSameDate(a.registeredDate, d),
      );
      regSeries.add(dayApplicants.length.toDouble());
      matchedSeries.add(
        dayApplicants
            .where((a) => a.matchStatus == 'matched')
            .length
            .toDouble(),
      );
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      items: ['All', ...kBarangays]
                          ...
                          .toList(),
                      onChanged: (v) =>
                          setState(() => _barangayFilter = v ?? 'All'),
                    ),
                  ),
                ),
              ],
            ...
            child: _SectionCard(
              ...
              child: LayoutBuilder(
                builder: (context, c) {
                  ...
                  return stacked
                      ? Column(
                          children: [donut, const SizedBox(height: 20), legend],
                        )
                      : Row(
                          children: [
                            donut,
                      ...
                      .map(
                        (a) => DataRow(
                          cells: [
                            ...
                            DataCell(
                              _StatusBadge(kMatchStyles[a.matchStatus]!),
                            ),
                            DataCell(
                              a.resumeSubmitted
                                  ? IconButton(
...
class _UserManagementSectionState extends State<UserManagementSection> {
  ...
  Future<void> _confirmDelete(JobSeekerUser user) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => _DeleteUserDialog(user: user),
    );
    if (confirmed == true) {
                            ...
                            .map(
                              (u) => DataRow(
                                cells: [
                                  ...
                                  DataCell(
                                    _StatusBadge(kUserStatusStyles[u.status]!),
                                  ),
                                  DataCell(
                                    Row(
                                      mainAxisSize: MainAxisSize.min,
            ...
            child: _SectionCard(
              ...
              child: _auditLog.isEmpty
                  ? const Padding(
                      ...
                      child: Center(
                        child: Text(
                          'No account actions yet. Actions you take above will appear here.',
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                          value: 'all',
                          child: Text('All concerns'),
                        ),
                      ],
                      onChanged: (v) =>
                          setState(() => _filter = v ?? 'unresolved'),
                    ),
                  ),
                ),
              ],
          ...
          children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 2),
                      Text(
                        'Brgy. ${c.barangay} \u00b7 Target: ${c.targetJob} \u00b7 ${_fmtDate(c.submittedDate)}',
                        style: const TextStyle(
                          fontSize: 11,
                          color: _C.slate500,
                        ),
            ...
            Align(
              ...
              child: TextButton.icon(
                ...
                icon: Icon(
                  c.resolved ? Icons.replay : Icons.check_circle_outline,
                  size: 15,
                  color: c.resolved ? _C.slate500 : _C.emerald600,
                ),
                label: Text(
                  c.resolved ? 'Reopen' : 'Mark as Resolved',
                  style: TextStyle(
                    fontSize: 12,
                    fontWeight: FontWeight.w700,
                    color: c.resolved ? _C.slate500 : _C.emerald600,
...
class JobBatchIngestionSection extends StatefulWidget {
  ...
  @override
  State<JobBatchIngestionSection> createState() =>
      _JobBatchIngestionSectionState();
}
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'File name',
            hintText: 'e.g. Montalban_Batch_003.xlsx',
          ),
        ),
        actions: [
          TextButton(
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: _C.primary.withValues(alpha: _hover ? 0.5 : 0.25),
                      width: 2,
                    ),
                    borderRadius: BorderRadius.circular(24),
                  ),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 3),
                                  Text(
                                    '${log.employer} \u00b7 Batch: ${log.batch} | Uploaded: ${log.uploaded}',
                                    style: const TextStyle(
                                      fontSize: 11,
                                      color: _C.slate500,
                                    ),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(width: 4),
                                      Text(
                                        'Matched ${log.matchedAccounts} job seeker account(s)',
                                        style: TextStyle(
                                          fontSize: 11,
                                          fontWeight: FontWeight.w700,
                                          color: _C.emerald600,
...
class _DoleSprsExportSectionState extends State<DoleSprsExportSection> {
  ...
  void _generateExcel() {
    if (_matched.isEmpty) {
      showToast(
        context,
        'No matched candidates to export yet.',
        icon: Icons.error_outline,
        iconColor: _C.rose,
      );
      return;
    ...
    showToast(
      context,
      'Generated DOLE SPRS export for ${_matched.length} candidate(s).',
    );
  }
  ...
  void _handoff() {
    ...
    if (pending.isEmpty) {
      showToast(
        context,
        'Everyone here has already been handed off to PESO Admin.',
      );
      return;
    }
    ...
    showToast(
      context,
      'Handed off ${pending.length} candidate(s) to PESO Admin.',
    );
  }
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text(
                      'Showing ${_matched.length} Matched Candidates',
                      style: const TextStyle(
                        fontWeight: FontWeight.w700,
                        fontSize: 13,
                        color: _C.slate600,
                ...
                child: Padding(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: isHandedOff
                            ? _C.emerald.withValues(alpha: 0.4)
                            : _C.slate200,
                      ),
                      borderRadius: BorderRadius.circular(20),
                      boxShadow: [
                              ...
                              RichText(
                                text: TextSpan(
                                  ...
                                  children: [
                                    ...
                                    TextSpan(
                                      text:
                                          '${a.yearsExperience} yrs experience',
                                      style: const TextStyle(
                                        fontWeight: FontWeight.w700,
                                        color: _C.slate700,
                                      ),
...
class PlatformAnalyticsSection extends StatefulWidget {
  ...
  @override
  State<PlatformAnalyticsSection> createState() =>
      _PlatformAnalyticsSectionState();
}
...
class _PlatformAnalyticsSectionState extends State<PlatformAnalyticsSection>
  ...
  bool get _hasUnsaved =>
      _minYears != _savedMinYears ||
      _confidence != _savedConfidence ||
      _maxMatches != _savedMaxMatches;
  ...
  @override
  Widget build(BuildContext context) {
    final resumesProcessed = widget.applicants
        .where((a) => a.resumeSubmitted && a.formSubmitted)
        .length;
    final autoMatched = widget.applicants
        .where((a) => a.matchStatus == 'matched')
        .length;
    final pendingReview = widget.applicants
        .where((a) => a.matchStatus == 'pending')
        .length;
                  ...
                  LayoutBuilder(
                    builder: (context, c) {
                      ...
                      final sliders = [
                        ...
                        _settingSlider(
                          'Max matches shown per job seeker',
                          Icons.list_alt_outlined,
                          _maxMatches,
                          1,
                          10,
                          (v) => setState(() => _maxMatches = v),
